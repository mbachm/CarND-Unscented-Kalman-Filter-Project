...
void UKF::Initialize(MeasurementPackage meas_package) {
  ...
  if (meas_package.sensor_type_ == MeasurementPackage::RADAR) {
    ...
    const double phi = normalize_angle(meas_package.raw_measurements_[1]);
    const double rhodot = meas_package.raw_measurements_(2);
    const double px = rho * cos(phi);
    const double py = rho * sin(phi);
    // Not needed anymore, see line 147
    //const double vx = rhodot * cos(0);
    //const double vy = rhodot * sin(0);
    
    //initialize state
    x_[0] = px;
    x_[1] = py;
    //sqrt(cos^2(phi) * rho_dot^2 + sin^2(phi) * rho_dot^2) = sqrt(rho_dot^2 * (cos^2(phi) + sin^2(phi)) = sqrt(rho_dot^2) = abs(rho_dot)
    x_[3] = abs(rhodot);
 ...
 * As suggested by the review, I use the EKF implementation for LIDAR as it leads to the
 * same results with less computation.
 * @param {MeasurementPackage} meas_package
 */
...
void UKF::AugmentedSigmaPoints(MatrixXd* Xsig_aug) {
  ...
  const double c0 = sqrt(lambda_ + n_aug_);
  for (int i = 0; i< n_aug_; i++)
  {
    Xsig_aug->col(i + 1)          = x_aug + c0 * L.col(i);
    Xsig_aug->col(i + 1 + n_aug_) = x_aug - c0 * L.col(i);
  }
  return;
}
...
void UKF::SigmaPointPrediction(MatrixXd* Xsig_out, double delta_t) {
  ...
  for (int i = 0; i< 2 * n_aug_ + 1; i++)
  {
    ...
    const double p_x = Xsig_out->coeffRef(0,i);
    const double p_y = Xsig_out->coeffRef(1,i);
    const double v = Xsig_out->coeffRef(2,i);
    const double yaw = Xsig_out->coeffRef(3,i);
    const double yawd = Xsig_out->coeffRef(4,i);
    const double nu_a = Xsig_out->coeffRef(5,i);
    const double nu_yawdd = Xsig_out->coeffRef(6,i);
    ...
    //add noise
    const double c0 = 0.5 * nu_a * delta_t *delta_t;
...
void UKF::PredictRadarMeasurement(VectorXd* z_out, MatrixXd* S_out, MatrixXd *Zsig_out) {
  ...
  for (int i = 0; i < 2 * n_aug_ + 1; i++) {  //2n+1 simga points
    ...
    const double yaw = normalize_angle(Xsig_pred_(3,i));
    ...
    //phi
    if (p_x == 0 && p_y == 0) {
      cout << "atan2(0,0) is undefined, set phi for col " << i << " to 0" << endl;
      Zsig(1,i) = 0;
    } else {
